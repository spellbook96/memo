...
/* 行列Aと行列Bの和を求める関数 */
void matrix_add(int matrix_a[ROW][COLUMN], int matrix_b[ROW][COLUMN], int matrix_c[ROW][COLUMN],int matrix_x[ROW][COLUMN]);
...
int main(void){
  ...
  printf("行列Xの要素を入力して下さい\n");
  for(i = 0; i < ROW; i++)
    for(j = 0; j < COLUMN; j++){
      /* 行列Bの要素を標準入力から入力し，matrix_bに格納する */
      printf("matrix_x[%d][%d]:", i, j);
      scanf("%d", &matrix_x[i][j]);
    }
  
  /* 行列Aと行列Bの和を求める関数を呼び出す */
  matrix_add(matrix_a, matrix_b, matrix_c,matrix_x);
...
void matrix_add(int matrix_a[ROW][COLUMN], int matrix_b[ROW][COLUMN], int matrix_c[ROW][COLUMN],int matrix_x[ROW][COLUMN]){
  ...
  for(i = 0; i < ROW; i++)
    for(j = 0; j < COLUMN; j++)
      ...
      matrix_c[i][j] = matrix_a[i][j] + matrix_b[i][j]+ matrix_x[i][j]; 
